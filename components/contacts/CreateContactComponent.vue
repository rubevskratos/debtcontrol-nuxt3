<template>
  <v-dialog v-model="dialog" width="30%">
    <template v-slot:activator="{ props }">
      <v-btn color="primary" v-bind="props"><v-icon>mdi-plus-outline</v-icon> Nuevo Contacto</v-btn>
    </template>

    <v-card>
      <v-card-text>
        <v-form>
          <v-text-field v-model="inputData.ContactDepartment" label="Departamento" />
          <v-text-field
...
export default {
  ...
  methods: {
    async updateContact() {
        ...
        options.body.Customer_id = this.customer.CustomerId
        console.log(options)
        try {
            let response = $fetch(`/api/customer_contact`, options)
            this.dialog = false
